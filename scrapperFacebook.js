...
export const scrapperFacebook = async () => {
	...
	const urls = [
		"https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=AR&media_type=all&search_type=page&view_all_page_id=427801784046327",
	];
	...
	try {
		for (const url of urls) {
			...
			await page.waitForSelector("div.xh8yej3", {
				timeout: 15000,
			});

			// Verifica si los resultados están presentes
			const resultadosDisponibles = await page.$(
				"div.xh8yej3"
			);
			...
			do {
				...
				const products = await page.evaluate(() => {
                    ...
                    const resultados = document.querySelectorAll("div.xh8yej3");
					let data = [];
		...
		console.log(
			"Error corriendo Puppeteer en scrapperFacebook.js:",
			error.message
		);
	} finally {
		// Cierra el navegador
